...
**Engine** — The vendored deterministic code. 16 command-line surfaces: structural validator, value validator, preflight, resolver, survey map, reverse audit, log-entry helper, document ingest, phoenix events, derived layer, commit gate, staged attribution, subject metadata lookup, governed subject queries, disposable subject-view artifacts, intent-plan validation. See the exact [command inventory](README.md#the-engine) and [delivery availability](docs/pr-delivery/README.md).
