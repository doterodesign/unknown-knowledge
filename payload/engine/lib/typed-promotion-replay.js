/** Fixed typed-current promotion recipe; generic query and delta rules stay unchanged. */
...
const verified = (id, outcome) => outcome?.eligible === true && outcome.verification === 'verified'
  && outcome.resolution.status === 'resolved' && outcome.resolution.policy === 'current' && outcome.resolution.requestedId === id;
...
function stableIneligible(id, before, after, declarations) {
  ...
  return before?.eligible === false && before.verification === 'not-required' && r?.status === 'unresolved'
    && r.policy === 'current' && r.requestedId === id && r.redirects.length === 0 && before.code === r.code
    && ((r.code === 'subject-suppressed' && s.status === 'suppressed')
      || (r.code === 'subject-retired' && s.status === 'retired' && s.retirement?.kind !== 'split')
...
export function compareTypedPromotionReplays(input) {
  ...
  if (!closed(input, ['version', 'before', 'after', 'limits', 'queryBudgets']) || input.version !== 1
    ...
    return fail('invalid-promotion-replay-input', 'Supply both actual contexts and explicit recipe/query capacities.');
  }
  try {
    const registries = {};
    for (const side of ['before', 'after']) {
      ...
      if (!binding.ok) return fail('promotion-replay-context-mismatch', 'Each context must retain its actual governance binding.', { side, diagnostics: binding.diagnostics });
    ...
    if (!stores.length || stores.some((name) => ['before', 'after'].some((side) => input[side].context.model.stores[name]?.present !== true))) {
      return fail('promotion-replay-store-unavailable', 'Every store present on either side is required on both sides.');
    }
    if (ids.length > input.limits.maxSubjects) return fail('promotion-replay-subject-budget', 'Reserve the complete actual canonical Subject union before eligibility.');
    ...
    for (const id of ids) {
      ...
      for (const side of ['before', 'after']) {
        ...
        try {
          ...
          return fail('promotion-replay-eligibility-unavailable', 'No failed eligibility call may shrink the operand inventory.', { id, side, ownerCode: error.code });
        }
      }
      if (verified(id, row.before) && verified(id, row.after)
        && row.before.resolution.id === id && row.after.resolution.id === id
      ...
      } else if (stableIneligible(id, row.before, row.after,
    ...
    if (result.subjects.some(({ disposition }) => disposition === 'blocked')) {
      return fail('promotion-replay-eligibility-unavailable', 'Every original operand must have verified current eligibility or an identical allowed ineligible outcome.');
    }
    const pairs = eligible.slice(1).map((id, i) => [eligible[i], id]);
    const count = BigInt(stores.length) * 4n * (4n + 2n * BigInt(eligible.length) + 4n * BigInt(pairs.length));
    if (count * 2n > BigInt(Number.MAX_SAFE_INTEGER)) return fail('promotion-replay-case-budget', 'The full paired workload exceeds safe representation.');
    result.resources.inventory.requiredCases = Number(count); result.resources.inventory.requiredQueryCalls = Number(count * 2n);
    if (count > BigInt(input.limits.maxCases)) return fail('promotion-replay-case-budget', 'The full paired recipe must fit before queries run.');
