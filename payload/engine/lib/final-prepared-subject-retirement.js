...
/** Fresh subject-retirement publication proof; the original owner report stays unchanged. */
...
import { decodePreparedSubjectRetirement, validRetirementCaptureLimits, isPreparedSubjectRetirementReport } from './prepared-subject-retirement.js';

const policyBytes = readFileSync(new URL('../policies/candidate-publication.json', import.meta.url));
const policy = JSON.parse(policyBytes).operations['subject-retirement'];
...
function parse(bytes, canonical = true) {
  ...
  catch { throw new EngineRefusal('invalid subject retirement JSON'); }
  if (canonical && !canonicalJsonBytes(value).equals(bytes)) throw new EngineRefusal('noncanonical subject retirement JSON');
  return value;
}

export async function runFinalPreparedSubjectRetirementGate(input) {
  const result = { version: 1, kind: 'final-prepared-subject-retirement', policy: { id: policy.id, version: policy.version, digest: canonicalSha256(policy) },
  ...
  if (!closed(input, ['repoRoot', 'evidenceDirectory', 'validationBundleDigest', 'expected', 'approvedRuntimeProfile', 'limits'])
    ...
    || input.expected?.operation !== 'subject-retirement' || !closed(input.limits, ['evidence', 'runtime'])
    ...
    || input.limits.runtime.maxCheckMilliseconds > 600000) return fail('invalid-final-subject-retirement-input');
  ...
  try {
    ...
    if (retained.status !== 'verified') return fail('retirement-retention-unavailable');
    ...
    if (!member('runtime/files/engine/policies/candidate-publication.json')?.bytes.equals(policyBytes)) return fail('retirement-policy-mismatch');
    ...
    if (!inputArtifact || !limitsArtifact || !rawReport) return fail('retirement-input-unavailable');
    const operationInputs = { gateInput: parse(inputArtifact.bytes), captureLimits: parse(limitsArtifact.bytes) };
    if (!validRetirementCaptureLimits(operationInputs.captureLimits)) return fail('retirement-capture-limits-invalid');
    const admitted = lifecycleMaterialPresent(operationInputs.gateInput) ? operationInputs.gateInput
      : decodePreparedSubjectRetirement(plan.repoRoot, operationInputs.gateInput);
    if (!same(admitted.before, result.source) || !same(admitted.candidate, result.candidate)
      || !same(admitted.impact.routes, { kind: 'runtime-capability' })) return fail('retirement-input-mismatch');
    const original = parse(rawReport.bytes, false);
    if (!isPreparedSubjectRetirementReport(original, { source: result.source, candidate: result.candidate }, operationInputs.gateInput) || !original.ok
      || original.inputDigest !== canonicalSha256(operationInputs.gateInput)) return fail('retirement-owner-report-invalid');
    ...
    if (retained.report.provenance !== 'verified' || retained.report.runtimeVerification !== 'verified'
      ...
      || !retained.report.checks.every((check) => check.status === 'passed' && check.completion === 'complete' && check.exitCode === 0)) return fail('retirement-validation-incomplete');
    const zero = admitted.operation.assignmentEvent === null;
    const captures = { registry: null, event: null };
    if (zero && member('checks/operation/event.yaml')) return fail('retirement-unexpected-event-capture');
    for (const kind of zero ? ['registry'] : ['registry', 'event']) {
      ...
      if (!row || row.size > cap || row.sha256 !== locator?.sha256) return fail('retirement-capture-mismatch');
    ...
    if (capability.status !== 'established' || capability.persistence !== 'unsupported' || capability.scope !== scope
      || capability.runtimeDigest !== retained.manifest.runtimeDigest || !same(capability.captures, { source: result.source, candidate: result.candidate })) return fail('retirement-capability-unavailable');
    plan.repoRoot = realpathSync(plan.repoRoot); work = mkdtempSync(join(realpathSync('/tmp'), 'final-subject-retirement-'));
    if (within(plan.repoRoot, work)) return fail('retirement-runtime-overlap');
    const runtime = capturePreparedRuntime(work, plan.limits.runtime, 'subject-retirement');
    if (Object.values(runtime.manifest.executables).some(({ path }) => within(plan.repoRoot, path))) return fail('retirement-runtime-overlap');
    if (canonicalSha256(runtime.manifest) !== retained.manifest.runtimeDigest) return fail('retirement-runtime-mismatch');
    if (!runtime.manifest.files.some(({ path }) => path === 'engine/lib/final-subject-retirement-check.js')) return fail('retirement-entrypoint-unavailable');
    ...
    const actual = await executePreparedWorker(runtime, jobFile, plan.limits.runtime, 'final-subject-retirement');
    if (actual.error || actual.status !== 0 || actual.signal || actual.stderr.length) return fail('retirement-worker-incomplete');
    ...
    if (!closed(output, ['version', 'gate', 'captures']) || output.version !== 1
      || !isPreparedSubjectRetirementReport(output.gate, { source: result.source, candidate: result.candidate }, operationInputs.gateInput)) return fail('retirement-output-invalid');
    ...
    if (!output.gate.ok || !same(output.gate, original) || !same(output.captures, captures)) return fail('retirement-fresh-proof-mismatch');
    result.status = 'passed'; return result;
  } catch (error) {
    if (!(error instanceof EngineRefusal) && !Number.isInteger(error?.errno)) throw error;
    return fail('retirement-evidence-unavailable');
  } finally {
    if (work) try { rmSync(work, { recursive: true, force: true }); }
    catch (error) {
      if (!Number.isInteger(error?.errno)) throw error;
      result.status = 'failed'; fail('retirement-cleanup-failed');
    }
  }
}
