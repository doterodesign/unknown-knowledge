...
import { capturePreparedSubjectMetadata } from './prepared-subject-metadata.js';
...
if (!same(gateInput.before, job.source) || !same(gateInput.candidate, job.candidate)) {
  throw new EngineRefusal('final metadata: descriptor mismatch');
}
const gate = await runPreparedSubjectMetadataGateFromWire({ repoRoot: job.repoRoot, gateInput });
let captures = null;
if (gate.ok) {
  const captured = await withTreeSnapshot(job.repoRoot, job.candidate.tree, ({ root }) => capturePreparedSubjectMetadata({
