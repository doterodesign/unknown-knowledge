/** Real fresh reconsideration proof and two candidate captures; failure has no captures. */
import { readFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { isDeepStrictEqual as same } from 'node:util';
import { canonicalJsonBytes } from './canonical-json.js';
import { verifyPreparedRuntime } from './prepared-runtime.js';
import { capturePreparedSubjectReconsideration } from './prepared-subject-reconsideration.js';
...
if (!same(gateInput.before, job.source) || !same(gateInput.candidate, job.candidate)) {
  throw new EngineRefusal('final reconsideration: descriptor mismatch');
}
const gate = await inspectSubjectReconsiderationGateFromWire({ repoRoot: job.repoRoot, gateInput });
let captures = null;
if (gate.ok) {
  const captured = await withTreeSnapshot(job.repoRoot, job.candidate.tree, ({ root }) => capturePreparedSubjectReconsideration({
