...
/** Actual immutable Git proof for one ordinary fresh Subject; no publication authority. */
...
import { admitSubjectCreationInput, decodeSubjectCreationInput } from './subject-creation-input.js';
import { createSubjectValidationBudget, adaptDocumentBudgetError } from './subject-validation-budget.js';
import { validateSubjectOrdinaryCreation } from './subject-governance.js';
...
export async function inspectSubjectCreationCore(rawInput) { return inspectCore(rawInput, false); }

/** Fixed internal gate handoff, produced only by this actual owner invocation. */
export async function inspectSubjectCreationGateCore(rawInput) { return inspectCore(rawInput, true); }

/** Fixed wire owner; its bundle is created here, never accepted from callers. */
export async function inspectSubjectCreationGateCoreFromWire(input) { return inspectCore(input, true, true); }

async function inspectCore(rawInput, forGate, wire = false) {
  const result = { version: 1, kind: 'subject-creation-core', ok: false, publicationReady: false,
  ...
  try {
    ...
    const admitted = wire
      ? decodeSubjectCreationInput(rawInput.repoRoot, { version: 1, before: rawInput.before, candidate: rawInput.candidate,
        operation: rawInput.operation, limits: rawInput.limits, evidence: rawInput.evidence }, { operationBudget: budget })
      : admitSubjectCreationInput(rawInput, { operationBudget: budget });
    ...
    for (const side of sides) {
      budget.charge('validationSteps', 1, 'creation-commit-membership');
      requireThat(readCommittedTree(input.repoRoot, input[side].commit).tree === input[side].tree,
        'creation-commit-tree', 'Each declared tree must belong to the exact supplied commit.');
    }
    requireThat(input.before.kitPath === input.candidate.kitPath, 'creation-kit-path', 'The installation location cannot change.');
      ...
      await withTreeSnapshot(input.repoRoot, input.candidate.tree, async candidate => {
        ...
        for (const side of sides) {
          ...
          requireThat((relative(snapshots[side].root, kitRoot) || '.') === input[side].kitPath,
            'creation-kit-path', 'Declared installation must equal the actual snapshot installation.');
          models[side] = loadStores(kitRoot);
          requireThat(models[side].ok, 'creation-model-unavailable', 'Both actual snapshots must be healthy models.');
        }
        const pair = originalPair(input, budget), { id, proposal, subject, registryEvent } = input.operation;
        const proof = validateSubjectOrdinaryCreation({ beforeModel: models.before, candidateModel: models.candidate,
          ...
          operation: { id, action: input.operation.action, proposal, subject, registryEvent }, allocationLimits: input.limits.allocation }, { operationBudget: budget });
        ...
        for (const [part, actual] of [['registry', registry.before], ['identity', identity.before]]) {
          requireThat(same(actual.locator, pair[part].capture) && actual.objectFormat === pair[part].objectFormat
            && actual.bytes.equals(pair[part].bytes), 'creation-original-before-membership', 'Original evidence must equal actual before bytes and full source locator.');
        }
        ...
        budget.charge('validationSteps', prior.subjects.length, 'creation-complete-subject-preservation');
        ...
        'creation-unrelated-registry-change', 'Only the exact selected consumption, fresh state and activation append may change the registry.');
        ...
        'creation-authorizer-changed', 'The whole current authorizer file must remain equal to the selected reviewed bytes.');
        ...
        requireThat(same(paths, [file('_identity.yaml'), file('subjects/registry.yaml')].sort()),
          'creation-changed-paths', 'Exactly the registry and identity paths may change; no assignment event is created.');
        ...
        for (const side of sides) {
          ...
          requireThat(!errors.length && !values.findings.some(row => row.severity === 'error') && !values.hardErrors.length && (!chain || chain.ok),
            'creation-structure-or-values', 'Actual stored structures, values and existing assignment history must pass.');
        }
        budget.assertActive();
      });
    });
    ...
    result.diagnostics.push({ code: error.code ?? 'creation-source-unavailable', path: '', message });
  } finally {
