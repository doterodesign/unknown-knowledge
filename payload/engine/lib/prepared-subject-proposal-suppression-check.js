...
if (!same(gateInput.before, job.source) || !same(gateInput.candidate, job.candidate)) {
  throw new EngineRefusal('prepared proposal suppression: descriptor mismatch');
}
const result = await runPreparedSubjectProposalSuppressionGateFromWire({ repoRoot: job.repoRoot, gateInput });
