#!/usr/bin/env node
/**
 * engine/ask.js — the entry shim (UCS-956 pattern).
 *
 ...
 *
 * The command lives in commands/ask.js. Its invocation path is unchanged.
 */
try {
  ...
  process.exitCode = await boot('ask', command);
} catch (error) {
  ...
  process.stderr.write(`ask: internal failure — the engine could not be loaded\n${error?.stack ?? error}\n`);
  process.exitCode = 2;
}
