...
**First action: read this contract before recursive filename or content
discovery, including searches through the installed kit, engine and dependency
directories.** Locating and reading top-level agent instructions and necessary
top-level configuration is permitted. Use their supplied protocol path rather
than recursively searching for runtime files. Before reading product source,
...
- **Reference by concept ID and path, never by copied value.** Anything you
  write into a store or a log carries `O-NNNNNN` / `D-NNNNNN` / leaf accession
  (`K-NNNNNN`) and
...
- **Trust is per-run.** A verdict is valid for the run that computed it;
  never cache or carry one across sessions (D-000011 — a stale "trusted" is a
  false all-clear).
...
**never cite a synthesized call number** (`SPO/ODD/REF·K-000001`) — it is a
...
residue as grammar merely to make discovery look complete. Near-miss entries
and excluded leaves are navigation diagnostics, not eligible evidence. If
candidates are irrelevant, ambiguous, excluded or insufficient, preserve that
reason and recover only the unresolved portion through the existing path below;
do not silently relax a constraint or treat a phrase binding as a complete answer.
...
(empty permits legacy unrestricted inclusion, not verified universal source
support) and inspect stage and freshness before selecting
...
For recovery findings, `consulted` accepts `concepts` (`O-NNNNNN`) and `leaves`
(`K-NNNNNN`) only. Decision paths belong in `summary`; there is no
...
```sh
node unknown-knowledge/engine/preflight.js --concepts O-000001,O-000002 --leaves K-000001,K-000002 --today <YYYY-MM-DD> --log --json --root .
```

Leaf-only request — no ontology hit is needed to check knowledge:

```sh
node unknown-knowledge/engine/preflight.js --leaves K-000001 --today <YYYY-MM-DD> --log --json --root .
```
...
```
node unknown-knowledge/engine/validate.js --concepts O-000001,O-000008 --root .
node unknown-knowledge/engine/validate-values.js --concepts O-000001,O-000008 --root .
```
...
staged diff. Attribution never limits the gate to a concept subset (D-000012).
...
## Conduct-on-verdict policy (D-000011)
...
- **Never wire a parser you authored into the validator in the same session**
  (D-000005). Unextractable anchors go to the miss-log; the governed path is
...
node unknown-knowledge/engine/log-entry.js create --log findings --date 2026-07-08 \
  ...
  --entry '{"trigger":"correction","summary":"correction: O-000001 src/registry/export-formats.ts","consulted":{"concepts":["O-000001"]}}'
```
...
1. **Draft** a decision entry in `decisions/entries/` with a provisional
   qualified id (`proposal:decision:<lowercase-v4-uuid>`), `status: proposed`, and
...
3. **Human gate**: acceptance requires a reviewed publication that allocates
   the final `D-NNNNNN` in this installation’s `_identity.yaml` and rewrites
   the exact proposal references together. A proposal consumes no slot;
   class names and filenames never assign identity. An accepted Decision
   alone is not approval of arbitrary candidate bytes. Status moves
