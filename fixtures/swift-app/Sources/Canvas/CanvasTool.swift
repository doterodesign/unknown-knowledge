import Foundation

/// The canvas-tool registry — the idiomatic Swift anchor for `swift-enum` (PRD §5.1).
/// Raw values are keyboard shortcuts and deliberately differ from the case names, so
/// the case-name facet and the raw-value facet are two distinct value sets (§3.5 emit).
///
/// ADVERSARIAL-BUT-EXTRACTABLE: interior comments (one containing the word
/// `case`), aligned `=` padding, a trailing comment with a stray `"` quote, a
/// decoy sibling enum in the same file, and a computed property whose `switch`
/// arms look like `case` declarations to a naive regex. A correct extractor
/// parses all of it; none of it is out-of-envelope.
enum CanvasTool: String, CaseIterable {
    case select = "V"
    // A comment between cases must not break extraction; neither must the
    // word `case` appearing inside it — case in point.
    case frame = "F"
    case pen        = "P" // aligned '=' padding
    case textStyle  = "T"
    case hand = "H" // trailing comment with a stray " quote
    case comment = "C"

    /// Switch arms below (`case .select:` …) must NOT be counted as enum
    /// cases — they sit inside the enum body but are pattern matches.
    var displayName: String {
        switch self {
        case .select: return "Select"
        case .frame: return "Frame"
        case .pen: return "Pen"
        case .textStyle: return "Text"
        case .hand: return "Hand"
        case .comment: return "Comment"
        }
    }
}

/// Decoy: a second enum in the same file. An extractor scoped to symbol
/// `CanvasTool` must not bleed `pointer` / `drawing` into its value set.
enum ToolGroup: String {
    case pointer = "pointer"
    case drawing = "drawing"
}
