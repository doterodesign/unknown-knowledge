...
// same Store, produced contradictory outcomes: `--concepts " O-000001 "` passed
...
test('the grammar trims, drops empties, de-duplicates, and sorts stably', () => {
  assert.deepEqual(normalizeConceptIds([' O-000001 ', 'O-000001', '', '  ', 'O-999100']), ['O-000001', 'O-999100']);
  assert.deepEqual(normalizeConceptIds([]), []);
  assert.deepEqual(normalizeConceptIds(['  ']), []);
  // Stable: the same input always yields the same order.
  assert.deepEqual(normalizeConceptIds(['O-000003', 'O-000001']), normalizeConceptIds(['O-000001', 'O-000003']));
});

// ------------------------------------- the surfaces agree on the same table
//
// O-000001 is a clean, active concept in the TS fixture, so every surface should
// treat these spellings of it identically. The exit code is the contract.

const SPELLINGS = [
  ['bare', 'O-000001'],
  ['padded', ' O-000001 '],
  ['duplicated', 'O-000001,O-000001'],
  ['padded and duplicated', ' O-000001 , O-000001'],
  ['with an empty member', 'O-000001,'],
];
...
for (const [label, value] of SPELLINGS) {
  test(`every surface reads ${label} (${JSON.stringify(value)}) the same way`, () => {
    ...
    const bare = SURFACES.map((cli) => run(cli, '--concepts', 'O-000001').status);
...
test('a duplicated id is checked once, not twice', () => {
  const once = JSON.parse(run('validate-values.js', '--concepts', 'O-000001', '--json').stdout);
  const twice = JSON.parse(run('validate-values.js', '--concepts', 'O-000001,O-000001', '--json').stdout);
...
test('an id that does not exist still hard-errors on every surface', () => {
  ...
  for (const cli of SURFACES) {
    const r = run(cli, '--concepts', 'O-999999');
...
test('padding cannot smuggle an unknown id past any surface', () => {
  for (const cli of SURFACES) {
    assert.equal(run(cli, '--concepts', '  O-999999  ').status, 2, `${cli} must refuse a padded unknown id`);
  }
});
