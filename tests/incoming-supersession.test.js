...
const files = {
  'K-000001': 'knowledge/design-system/600.1-adding-a-new-token.md',
  'K-000003': 'knowledge/design-system/600.2-archived-theme-fallback-basis.md',
  'K-000004': 'knowledge/design-system/600.3-contrast-disclosure.md',
};
...
function scenario(t) {
  ...
  for (const [id, term] of [['K-000001', 'sunset policy'], ['K-000003', 'replacement guidance']]) {
    editLeaf(store, id, (leaf) => {
      leaf.terms = [term];
      leaf.concepts = [];
      leaf.operations = [];
      leaf.relates = id === 'K-000003' ? { supersedes: ['K-000001'] } : {};
    });
  }
  return store;
}
...
test('an old-only query exposes its direct successor without changing the matched result', (t) => {
  ...
  const before = readFileSync(join(store, files['K-000001']), 'utf8');
  const result = resolve(store, 'sunset policy');
  assert.deepEqual(result.leaves.map((leaf) => [leaf.id, leaf.score]), [['K-000001', 1]]);
  const [successor] = result.leaves[0]['superseded-by'];
  assert.equal(successor.id, 'K-000003');
  assert.equal(successor.file, files['K-000003']);
  ...
  assert.equal(readFileSync(join(store, files['K-000001']), 'utf8'), before, 'no authored reciprocal edge');
});

test('query, concept, path, document gather and scope exclusions carry the same successor navigation', (t) => {
  const store = scenario(t);
  editLeaf(store, 'K-000001', (leaf) => {
    leaf.concepts = ['O-000001'];
    leaf.applies = { jurisdictions: ['us-ca'] };
  });
  const expected = resolve(store, 'sunset policy').leaves[0]['superseded-by'];
  const concept = resolve(store, 'token').results.find((result) => result.id === 'O-000001');
  assert.deepEqual(concept.knowledge.find((leaf) => leaf.id === 'K-000001')['superseded-by'], expected);
  const paths = resolve(store, '--paths', 'src/registry/tokens.ts');
  assert.deepEqual(paths.paths[0].knowledge.find((leaf) => leaf.id === 'K-000001')['superseded-by'], expected);
  ...
  assert.deepEqual(scoped.leaves.map((leaf) => leaf.id), ['K-000001']);
  ...
  for (const args of [['sunset policy'], ['--paths', 'src/registry/tokens.ts'], ['--doc', document], ['sunset policy eu eaa']]) {
    ...
    assert.match(human.stdout, /superseded-by: K-000003/);
...
test('a supersession cycle stays bounded in resolution and fails target preflight through ref-cycle', (t) => {
  const store = scenario(t);
  editLeaf(store, 'K-000001', (leaf) => { leaf.relates.supersedes = ['K-000003']; });
  assert.deepEqual(resolve(store, 'sunset policy').leaves[0]['superseded-by'].map((leaf) => leaf.id), ['K-000003']);
  const result = spawnSync(process.execPath, [join(root, 'payload/engine/preflight.js'),
    '--leaves', 'K-000001,K-000003', '--root', store, '--today', TODAY, '--json'], { encoding: 'utf8' });
...
test('every member of overlapping supersession cycles fails even when preflighted alone', (t) => {
  const store = scenario(t);
  editLeaf(store, 'K-000001', (leaf) => { leaf.relates.supersedes = ['K-000003', 'K-000004']; });
  editLeaf(store, 'K-000004', (leaf) => { leaf.relates = { supersedes: ['K-000003'] }; });
  for (const id of ['K-000001', 'K-000003', 'K-000004']) {
...
test('chains are followed one hop at a time and multiple draft or stale successors remain explicit', (t) => {
  const store = scenario(t);
  editLeaf(store, 'K-000004', (leaf) => {
    leaf.terms = ['final instructions'];
    leaf.concepts = [];
    leaf.operations = [];
    leaf.relates = { supersedes: ['K-000003', 'K-000001', 'K-000001'] };
    leaf.facets.stage = 'draft';
  });
  editLeaf(store, 'K-000003', (leaf) => { leaf.verified = '2020-06-30'; });
  const result = resolve(store, 'sunset policy');
  const successors = result.leaves[0]['superseded-by'];
  assert.deepEqual(successors.map((leaf) => leaf.id), ['K-000003', 'K-000004']);
  ...
  assert.deepEqual(resolve(store, 'replacement guidance').leaves[0]['superseded-by'].map((leaf) => leaf.id), ['K-000004']);
  assert.deepEqual(resolve(store, 'final instructions').leaves[0]['superseded-by'], []);
  assert.deepEqual(resolve(store, 'historical sunset policy').leaves.map((leaf) => leaf.id), ['K-000001']);
  assert.deepEqual(resolve(store, 'sunset policy'), result, 'identical inputs produce identical bytes and order');
  const preflight = cli(store, 'preflight.js', ['--leaves', 'K-000003,K-000004'], 2);
...
test('dangling supersession retains structural health failure and invents no navigable stub', (t) => {
  const store = scenario(t);
  editLeaf(store, 'K-000001', (leaf) => { leaf.relates.supersedes = ['K-999999']; });
  const result = resolve(store, 'sunset policy');
  assert.equal(result['store-health'].ok, false);
  assert.deepEqual(result.leaves[0].relates.supersedes, []);
  assert.deepEqual(result.leaves[0]['superseded-by'].map((leaf) => leaf.id), ['K-000003']);
  const preflight = cli(store, 'preflight.js', ['--leaves', 'K-000003'], 2);
...
test('disposable derived files neither supply nor retain supersession', (t) => {
  ...
  assert.deepEqual(resolve(store, 'sunset policy'), before);
  editLeaf(store, 'K-000003', (leaf) => { leaf.relates = {}; });
