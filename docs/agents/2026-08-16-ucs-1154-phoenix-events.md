...
subtree segment-wise, so `design-system` claims `design-system/components` but
not `design-system-legacy`.
...
either round, and correct. P-001 sent BOTH `design-system/components` leaves to
`design-system/primitives` and the lone leaf to `design-system/patterns`:
...
perform. Fixed by making L-000133 (empty-state pattern guidance) MISFILED under
`design-system/components` in the before-state, which is both the honest story
and a better one: composition guidance filed under components because it arrived
alongside a component is exactly the drift a phoenix event exists to fix. Now one
predecessor branches to two successors and a class-level rule would be right
about two leaves and wrong about the third. Scope narrowed to `components` alone;
the stray successor dropped from the registry (no leaf left to warrant it);
