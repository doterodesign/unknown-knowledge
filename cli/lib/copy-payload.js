...
import { isIdentityUuid } from '../../payload/engine/lib/record-identity.js';
...
const IDENTITY_FILE = '_identity.yaml';
...
export function expandManifest(manifest, stacks = []) {
  ...
  for (const generated of [SEEDED_MANIFEST, IDENTITY_FILE]) {
    const clashes = (path) => path === generated || path.startsWith(`${generated}/`);
    if (plan.some(({ to }) => clashes(to)) || manifest.create.some(clashes)) {
      throw new Error(`manifest entry targets ${generated} — that file is generated by the engine (§9.1)`);
    }
  }
  return plan;
}
 ...
 * @param {string} options.namespace exact installation UUID injected by the caller
 ...
 */
export function copyPayload({ kitRoot, targetDir, namespace, rootName = DEFAULT_ROOT, stacks = [], manifestPath }) {
  if (!isIdentityUuid(namespace)) throw new SeedRefusal('namespace must be an exact lowercase version-4 UUID');
  ...
  writeFileSync(join(destRoot, IDENTITY_FILE),
    `schema-version: 1\nidentity-format: 1\nnamespace: ${namespace}\nallocations: []\n`);
  files.push(IDENTITY_FILE, SEEDED_MANIFEST);
  files.sort();
  writeFileSync(join(destRoot, SEEDED_MANIFEST),
    renderSeededManifest({ version, stacks: selected, zones: { ...manifest.zones,
      client: [...new Set([...(manifest.zones.client ?? []), IDENTITY_FILE])].sort() }, files }));
