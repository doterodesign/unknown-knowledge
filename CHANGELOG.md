...
- `acceptance/retrieval/heldout-run.js` runs the comparison (`--first-repeat` numbers later passes after earlier ones). It reads sealed cases from the custody directory and writes traces and grader reasoning only there; it reports verdict codes, critical flags, cost, time and output size.
