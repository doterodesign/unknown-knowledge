...
(`fixtures/swift-app`, `fixtures/ts-app`), plus UCS-1159's two isolated plant
stores (`fixtures/plant-duplicate-accession`, `fixtures/plant-unresolved-relates`
— knowledge-only stores that each fail to LOAD by design, one planted defect
apiece, asserted at exit 2 in §A3):

```sh
npm run acceptance        # = node acceptance/run.js
```

Zero-dependency node (D-000002); every assertion goes through the engine's
...
3. asserts nothing under `fixtures/` or `tests/` was vendored
   (acceptance-fixture leakage impossible by construction, D-000007), and that
   the selected stacks' extractor fixtures — and *only* those — are present
   (D-000009, per selection combination);
...
2. **auto-detection picks the right stack** per fixture (`ts` for `ts-app`,
   `swift` for `swift-app`), and only that stack's extractor pack ships (D-000009);
3. the **D-000009 later-stacks warning** appears in the output *and* in the
   seeded `README.md`;
...
5. **no CI file is ever written** (D-000006): the seeded tree carries no
   `.github/`.

Init also preflights the seeded engine's one runtime dependency (`js-yaml`,
D-000002), resolving it from the seeded engine file itself. Unresolvable is a
...
| [`A5-canonical-bootstrap-walkthrough.md`](A5-canonical-bootstrap-walkthrough.md) | Current phase-2 bootstrap: proposals, reviewed publication, then actual active-record checks | fresh Swift installation | manual; publication required for completion |
| [`A5-knowledge-bootstrap-walkthrough.md`](A5-knowledge-bootstrap-walkthrough.md) | Preserved July 2026 bootstrap command trace | swift-app | historical (KK-21), not current expected output |
