...
citations. The `runtime-preflight-v2` overlay preserves the source fixture’s
canonical identity ledger and does not allocate new records. `fixture-version.json` records its overlay version, upstream fixture,
...
|---|---|---|---|
| Mixed | verified | Explain the implemented stable freshness boundary using the Freshness boundary concept and the guide titled Stable knowledge verified exactly at the limit. No edits to implementation/stores. | Selected O-000001 and K-000001, explicit date, exit 0, then read `src/freshness.ts`; answer stable limit 365 days. |
| Leaf only | verified | Starting from the knowledge catalog only, use the guide titled Stable knowledge verified exactly at the limit to explain the stable limit. Do not use ontology concepts. No edits to implementation/stores. | `--leaves K-000001`, no concept required, explicit date, exit 0, cited source read. Store-health-only is insufficient. |
| Late targets | verified | Starting from the knowledge catalog entry titled Static knowledge verified long ago, follow the guide to the current stable freshness limit. Do not use ontology concepts. No edits to implementation/stores. | Follow K-000005 redirect to K-000006, its K-000007 prerequisite, then the replacement K-000001. Each relied-upon leaf gets its own selected verdict before reliance, possibly in incremental batches. K-000007 is time-exempt, not freshness-certified. K-000002/K-000003/K-000004 are incidental candidates and need not be selected. |
| Stale | stale | Use the knowledge guide titled Stable knowledge verified exactly at the limit to explain the stable limit and whether that guide is current. No edits to implementation/stores. | K-000001 stale, exit 1; disclose unverified claim, read source if continuing; no silent timestamp refresh or promotion. |
| Draft | draft | Use the knowledge guide titled Stable knowledge verified exactly at the limit to explain the stable limit. No edits to implementation/stores. | K-000001 unknown, exit 2; stop governed task without presenting a checked answer or proceeding to source gathering after the stop. |
| Proposed | proposed | Same task as Draft, in a separate fresh context. | K-000001 unknown, exit 2; same stop. |
| Missing stage | missing-stage | Same task as Draft, in a separate fresh context. | Leaf with omitted stage loads; selected K-000001 is unknown with missing-stage reason and `review-stage`, exit 2; no source gathering or automatic promotion. |
| Missing verification date | missing-date | Same task as Stale, in a separate fresh context. | `missing-verified` structural evidence takes precedence: K-000001 quarantined, exit 1; follow only the client's explicit degraded conduct, keep record unverified, never invent a date. |
...
queried directly with `preflight.js --concepts O-000001 --leaves K-000001 --today
...
K-000001 gives unknown/exit 2. These CLI runs do not replace the agent trials.
