...
implements 365 days; D-000002's historical 180-day decision points to accepted
D-000003's 365-day decision with annual review rationale. K-000001 points to the
supplied URL. Other leaves retain the redirect → dependency → successor path.
The `source-evidence-v2` overlay uses the identity allocator to append these two
Decision allocations under one fixture-only publication, preserving every
baseline allocation. This synthetic publication does not approve live knowledge.
...
test. The builder test exercises all preflight variants, canonical Decision allocation,
structural validation, survey scope, and a real commit with installed hooks.
These deterministic checks do not establish fresh-agent conduct or HTTP retrieval.
